...
class EditLabelsMenu extends React.PureComponent {
  ...
  handleDoneClick = async () => {
    if (this.state.labelName !== "") {
      var labelDetails = {
       ...
       await LabelController.addlabeltouser(labelDetails).then((res) => {
        if (res.status === 200) {
  ...
  render() {
    console.log("array is :", this.state.getLabelArr);
    let getAllLabels = this.state.getLabelArr.map((item2) => {
      return (
        <GetLabels
